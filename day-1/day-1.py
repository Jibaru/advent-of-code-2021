...
measurements = list(map(lambda num: int(num), data.split('\n')))
...
def solve02():
    max_measurement_index = len(measurements) - 1

    def get_sum_indexes(measurement_index):
        '''
        Check the sequence
        0 -> 0
        1 -> 0, 1
        ...
        if measurement_index == max_measurement_index:
            return [measurement_index - 2]

        if measurement_index == max_measurement_index - 1:
            return [measurement_index - 1, measurement_index - 2]

        return [measurement_index, measurement_index - 1, measurement_index - 2]

    sums = {}

    for index, measurement in enumerate(measurements):
        sum_indexes = get_sum_indexes(index)

        for sum_index in sum_indexes:
            if sum_index in sums:
                sums[sum_index] += measurement
            else:
                sums[sum_index] = measurement
